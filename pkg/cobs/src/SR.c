...
int CheckPositivity(int m, int ind[], double x[], double y[],
		    double delta[], double rind[], double tol);
...
/* {comment from Geurt's R code:}
 Needed function: s3D
 The function s3D solves a tridiagonal systeem op
		    ...
		    double delta[], double rind[], double tol)
{
    ...
    for (i=2;i<m;i++)
    {
	a=(rind[i+1]-rind[i])/delta[i]-(rind[i]-rind[i-1])/delta[i-1];
	if (a<min)
	{
	    min = a;
	    imin = i;
	...
	double *phi_BL, int *NumIt)
{
	...
	*rind = dvector(1,n);
    ...
    // all these arrays are 1-indexed {originally: created in main.c with
    // nrutil's  dvector(1,n)
    x--; y--; r--;
    R--; H--; S--; Y--; D--;
	...
	if (m>2)
	{
	    m = CheckPositivity(m,ind,x,y,delta,rind, tol_pos);
	    rindtor(m,x,ind,rind,r);
	}
...
void SR_R(int* n, double* cc, int *m1, int ind[],
	  ...
	  double *phi_BL, int *NumIt)
{
    SR(n[0], cc[0], m1, ind, x,y, r, R,H, S,Y,D, tol, it_max[0], phi_BL, NumIt);
}
